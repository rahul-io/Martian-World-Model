...
from dust3r.cloud_opt.base_opt import BasePCOptimizer, Base
...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def get_depthmaps(self, raw=False):
        ...
        if not raw: # reshape to H,W
            res = [dm[:h*w].view(h, w) for dm, (h, w) in zip(res, self.imshapes)]
        return res

    def depth_to_pts3d(self, extrinsic=None, depth=None):
        # Get depths and  projection params if not provided
        focals = self.get_focals()
        pp = self.get_principal_points()
        if extrinsic is not None:
            im_poses = torch.tensor(extrinsic).to(self.device)
            depth = np.array(depth)
            depth = depth.reshape(2, -1)
    ...
    def get_pts3d2(self, extrinsics, depth, raw=False):
        res = self.depth_to_pts3d(extrinsics, depth)
        if not raw:
            res = [dm[:h*w].view(h, w, 3) for dm, (h, w) in zip(res, self.imshapes)]
        return res

    def depth_to_pts3d_single(self, extrinsic=None, depth=None):
        focal = self.get_focals()[0:1]           
        pp = self.get_principal_points()[0:1]      
        grid = self._grid[0:1]                    

        im_poses = torch.tensor(extrinsic, dtype=torch.float32, device=self.device)
        if im_poses.ndim == 2:
            im_poses = im_poses.unsqueeze(0)      
        ...
        rel_ptmaps = _fast_depthmap_to_pts3d(depth, grid, focal, pp=pp)
        return geotrf(im_poses, rel_ptmaps)
    
    def get_pts3d_0(self, extrinsics, depth, raw=False):
        res = self.depth_to_pts3d_single(extrinsics, depth)
        if not raw:
            res = [dm[:h*w].view(h, w, 3) for dm, (h, w) in zip(res, self.imshapes)]
        return res
