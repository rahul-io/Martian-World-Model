...
from component.lib_bilagrid import total_variation_loss, slice
import uuid
try:
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = GaussianModel(dataset.sh_degree, use_bilateral_grid=dataset.use_bilateral_grid, bilateral_grid_shape=dataset.bilateral_grid_shape)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if dataset.use_bilateral_grid:
            grid_y, grid_x = torch.meshgrid(
                (torch.arange(gt_image.shape[0], device="cuda") + 0.5) / gt_image.shape[0],
                (torch.arange(gt_image.shape[1], device="cuda") + 0.5) / gt_image.shape[1],
                indexing="ij",
            )
            grid_xy = torch.stack([grid_x, grid_y], dim=-1).unsqueeze(0)
            image = image.permute(1, 2, 0)
            image = slice(gaussians.bil_grids, grid_xy, image.unsqueeze(0), torch.tensor(rand_idx, device="cuda"))["rgb"]
        ...
        if dataset.use_bilateral_grid:
            tvloss = 10 * total_variation_loss(gaussians.bil_grids.grids)
            loss += tvloss
        loss.backward()
        iter_end.record()
        ...
        with torch.no_grad():
            ...
            if iteration < opt.iterations:
                ...
                if dataset.use_bilateral_grid:
                    for optimizer in gaussians.bil_grid_optimizers:
                        optimizer.step()
                        optimizer.zero_grad(set_to_none=True)
                    gaussians.bil_grid_schedulers.step()
