...
def split_train_test(image_files, llffhold=8, n_views=None, verbose=True):
    test_idx  = np.linspace(1, len(image_files) - 2, num=1, dtype=int)
...
def load_images(folder_or_list, size, square_ok=False, verbose=True):
    ...
    for path in folder_content:
        img_names.append(os.path.join(root, path))
        ...
        if size == 224:
            ...
            img = _resize_pil_image(img, size) # 1096x905 -> 512x387
        W, H = img.size
        cx, cy = W//2, H//2
        if size == 224:
            half = min(cx, cy)
...
def save_points3D(sparse_path, imgs, pts3d, confs, masks=None, use_masks=True, save_all_pts=False, save_txt_path=None, depth_threshold=0.1, max_pts_num=150 * 10**10):
    ...
    if confs is None:
        confs = np.ones((imgs.shape[0], imgs.shape[1], imgs.shape[2]))
        confs = confs.reshape(pts3d.shape[0], -1)
    ...
    save_txt_path.mkdir(exist_ok=True, parents=True)
...
def compute_co_vis_masks(sorted_conf_indices, depthmaps, pointmaps, camera_intrinsics, extrinsics_w2c, image_sizes, depth_threshold=0.1):
    ...
    for i, curr_map_idx in tqdm(enumerate(sorted_conf_indices), total=len(sorted_conf_indices)):
        ...
        idx_before = sorted_conf_indices[:i].astype(np.int64)
...
def warp_corners_and_draw_matches(ref_points, dst_points, img1, img2, num_matches_to_draw=500):
    ...
    # Randomly select a subset of matches to draw
    if len(matches) > num_matches_to_draw:
        matches = np.random.choice(matches, num_matches_to_draw, replace=False).tolist()
