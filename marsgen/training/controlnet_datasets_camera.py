...
class RealEstate10KPoseControlnetDataset(Dataset):
    def __init__(
        ...
        # self.dataset = json.load(open(os.path.join(root_path, file_name_txt), 'r'))
        self.dataset = json.load(open("/opt/data/private/llf/spacediffusion/marsvideodata/annotations/validation.json", 'r'))
        with open("../parabolic_select.txt", 'r') as f:
            parabolic_lines = [line.strip() for line in f.readlines()]
        allowed_prefix = {"_".join(line.split('_')[:2]) for line in parabolic_lines if len(line.split('_')) >= 2}
        self.dataset = [item for item in self.dataset if not ('parabolic' in item['clip_name'] and "_".join(item['clip_name'].split('_')[:2]) not in allowed_prefix)]
        self.dataset = [item for item in self.dataset if not('spiral' in item['clip_name'])]
        self.dataset = [item for item in self.dataset if not('circle' in item['clip_name'])]
        self.dataset = [item for item in self.dataset if not('ellipse' in item['clip_name'])]
    ...
    def load_video_reader(self, idx):
        ...
        return video_reader, video_dict['clip_name']
    ...
    def load_image(self,idx):
        ...
        image_path = os.path.join(self.root_path, 'images',f'{video_id}.png')
        image = Image.open(image_path)
        return image
    
    def load_prompt(self,idx):
        video_id= self.dataset[idx]
        prompt = video_id['caption']
        return prompt
    
    def get_batch(self, idx):
        video_reader, clip_name = self.load_video_reader(idx)
        ...
        if self.use_text_prompt:
            ...
            video_caption=""
        if self.all_length:
            self.sample_n_frames=len(video_reader) 
        ...
        intrinsics = np.asarray([[cam_param.fx /ori_w * self.sample_size[1],
                                  cam_param.fy /ori_h * self.sample_size[0],
                                  cam_param.cx /ori_w * self.sample_size[1],
                                  cam_param.cy /ori_h * self.sample_size[0]]
