...
from controlnet_datasets_camera import RealEstate10KPoseControlnetDataset
...
def get_optimizer_total_params(optimizer):
    total_params = 0
...
class MyModel(torch.nn.Module):
    def __init__(self, text_encoder,transformer,vae,controlnet):
        super().__init__()
...
def main(args):
    print(f"DeepSpeed enabled: {is_deepspeed_available()}")
    ...
    #print('use_deepspeed_optimizer:', use_deepspeed_optimizer)
    #print('use_deepspeed_scheduler:', use_deepspeed_scheduler)

    optimizer = get_optimizer(args, params_to_optimize, use_deepspeed=use_deepspeed_optimizer)

    # Dataset and DataLoader
    train_dataset = RealEstate10KPoseControlnetDataset(
        video_root_dir=args.video_root_dir,
        file_name_txt=args.file_name_txt,
    ...
    def collate_fn(examples):
        ...
        first_image=[example["first_image"] for example in examples]
        ...
        first_image = torch.stack(first_image)
        first_image=first_image.to(memory_format=torch.contiguous_format).float()
    ...
    # my_model = MyModel(text_encoder,transformer,vae,controlnet)
    # par_before=get_optimizer_total_params(optimizer)
    #print('par_before:',par_before)
    controlnet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        controlnet, optimizer, train_dataloader, lr_scheduler
    )
    # par_after=get_optimizer_total_params(optimizer)
    # print('par_after:',par_after)
    ...
    if accelerator.is_main_process:
        ...
        # accelerator.init_trackers(tracker_name, config=vars(args), init_kwargs={"wandb": {"name": "test"}})
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                controlnet_encoded_frames = batch["controlnet_videos"].to(dtype=weight_dtype)
                prompts = batch["prompts"]
                #print('prompts:',prompts)
                #process image
                images=batch["first_image"].to(dtype=weight_dtype)   # [B, C, H, W]   
                images = images.unsqueeze(1)  #BFCHW
                # Add noise to images
                image_noise_sigma = torch.normal(mean=-3.0, std=0.5, size=(1,), device=images.device)
                image_noise_sigma = torch.exp(image_noise_sigma).to(dtype=images.dtype)
                noisy_images = images + torch.randn_like(images) * image_noise_sigma[:, None, None, None, None]
                image_latents = encode_video(noisy_images).to(dtype=weight_dtype)
                #print('model_input shape:',model_input.shape)  #B F C H W
                #print('image_latents:',image_latents.shape) #B 1 C H W
                #sys.exit()

                padding_shape = (model_input.shape[0], model_input.shape[1] - 1, *model_input.shape[2:])
                latent_padding = image_latents.new_zeros(padding_shape)
                #print('latent_padding:',latent_padding.shape)  #([1, 12, 16, 60, 90])
                #print('image_latents:',image_latents.shape) #([1, 1, 16, 60, 90])
                image_latents = torch.cat([image_latents, latent_padding], dim=1)  #BFCHW
                ...
                #print('latent_img_noisy[:,:,:16,:,:]:',latent_img_noisy[:,:,:16,:,:].shape)
                # print('hideen_states dtype:',latent_img_noisy.dtype)
                # print('prompt_embeds dtype:',prompt_embeds.dtype)
                # print('image_rotary_emb dtype:',image_rotary_emb[0].dtype)
                # print('controlnet_states dtype:',controlnet_encoded_frames.dtype)

                controlnet_states = controlnet(
                    hidden_states=latent_img_noisy[:,:,:16,:,:],
                ...
                #output controlnet:
                # for i,contronlet_state in enumerate(controlnet_states):
                #     print(f'layer {i} mean:',contronlet_state.mean(),' min:',contronlet_state.min(),' max:',contronlet_state.max())
