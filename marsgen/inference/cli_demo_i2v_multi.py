...
import sys
sys.path.append('./ac3d')
sys.path.append('./ac3d/training')
...
def generate_video(
    ...
    - annotation_txt (str): Name of subset (train.json or test.json)
    ...
    - gpu_id (int): The ID of current GPU
    - num_gpus (int): Total number of GPUs to use
    ...
    # Move controlnet to specified device
    ...
    eval_dataset = RealEstate10KPoseControlnetDataset(
        ...
        file_name_txt=annotation_txt,
        image_size=(height, width), 
        stride=(stride_min, stride_max),
        sample_n_frames=num_frames,
    )
    
    # Calculate the range of indices for this GPU
    dataset_size = len(eval_dataset)
    
    #optional
    max_case=20
    if dataset_size>max_case:
        dataset_size=max_case
        
    indices_per_gpu = dataset_size // num_gpus
    start_idx = gpu_id * indices_per_gpu
    end_idx = (gpu_id + 1) * indices_per_gpu if gpu_id < num_gpus - 1 else dataset_size
    
    print(f"GPU {gpu_id} processing indices {start_idx} to {end_idx-1}")
    
    # Only process assigned range
    for camera_idx in range(start_idx, end_idx):
        ...
        #print('controlnet_latents:', controlnet_latents.shape)
        #print('reference_video:', reference_video.shape)
        images=data_dict['first_image']
        video_id=data_dict['video_id'].replace('\n','')

        # Set output directory
        os.makedirs(output_path,exist_ok=True)
        output_path_file = os.path.join(output_path, f"{video_id}_out.mp4")
        ...
        print('[video generated] ',  output_path_file)
        if save_ref:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", default="", help="The description of the video to be generated")
    parser.add_argument(
        "--video_root_dir",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--base_model_path", type=str, default="", help="The path of the pre-trained model to be used"
    )
    parser.add_argument(
    ...
    parser.add_argument("--annotation_txt", type=str, default='test.json', help="Subset of video_root_dir dataset")
    ...
    parser.add_argument("--stride_min", type=int, default=3)
    parser.add_argument("--stride_max", type=int, default=5)
    parser.add_argument("--height", type=int, default=480)
    parser.add_argument("--width", type=int, default=720)
    parser.add_argument("--num_frames", type=int, default=49)
    parser.add_argument("--start_camera_idx", type=int, default=0)
    parser.add_argument("--end_camera_idx", type=int, default=3)
    ...
    parser.add_argument("--num_gpus", type=int, default=1, help="Total number of GPUs to use")
    parser.add_argument("--save_ref", action="store_true", default=False, help=("Save reference video.")),

    args = parser.parse_args()
    
    # if args.output_path is None:
    #     ckpt_num=args.controlnet_model_path.split('checkpoint-')[-1].split('.pt')[0]
    #     args.output_path = f"./output/{ckpt_num}"
    ...
    generate_video(
        ...
        annotation_txt=args.annotation_txt,
