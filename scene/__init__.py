...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, opt=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if os.path.exists(os.path.join(args.source_path, f"sparse_{args.n_views}")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.depths, args.eval, args)
        ...
        if self.loaded_iter:
            ...
            self.gaussians.create_from_pcd(scene_info.point_cloud, self.cameras_extent, scale_gaussian=scale_gaussian)
